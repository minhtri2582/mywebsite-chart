S3 Enforce Object Ownership là một tính năng bảo mật trong Amazon S3 dùng để kiểm soát quyền sở hữu đối với các đối tượng (objects) được upload vào bucket — đặc biệt khi có nhiều tài khoản AWS khác nhau cùng tương tác với bucket đó.
...
⚙️ Các chế độ của “Object Ownership”

Chế độ	Mô tả	ACL bị vô hiệu?
Bucket owner enforced	Chủ bucket luôn là chủ sở hữu mọi object trong bucket. Bỏ qua mọi ACL.	✅ Có
Bucket owner preferred	Nếu uploader và owner khác tài khoản, S3 sẽ cố gắng gán quyền sở hữu cho chủ bucket (nếu có quyền).	❌ Không
Object writer	Người upload (uploader) là người sở hữu object.	❌ Không

👉 Từ năm 2023, AWS khuyến nghị mạnh mẽ sử dụng “Bucket owner enforced” để tránh rủi ro bảo mật và lỗi truy cập khi làm việc đa tài khoản (cross-account).
...
Bạn có muốn mình minh họa thêm diagram flow thể hiện sự khác biệt giữa trước và sau khi bật “Enforce Object Ownership” không?


ACL (Access Control List) trong Amazon S3 là một cơ chế kiểm soát quyền truy cập cấp độ đối tượng hoặc bucket. Nó định nghĩa ai có thể truy cập (AWS account hoặc public) và được phép làm gì (đọc, ghi, hoặc quản lý quyền) trên các đối tượng trong S3.
...
Bạn có muốn mình minh họa bằng sơ đồ so sánh ACL vs Bucket Policy không (rất trực quan)?
